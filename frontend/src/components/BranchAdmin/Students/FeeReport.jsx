  ...
  const fetchStudentById = async (sid) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Set default term to Term-1
        setSelectedTerm("Term-1");
        // Update due amounts based on Term-1
        setStudentDataForm((prev) => {
          const updatedPadiFee = prev.padiFee.map((fee) => {
            let termValue = 1; // Use Term-1 by default
            termValue > fee.terms ? (termValue = fee.terms) : (termValue = termValue);
            const dueAmount =
              termValue * fee.extractedAmount - fee.paidAmount > 0
                ? termValue * fee.extractedAmount - fee.paidAmount
  ...
  const handleEnteredAmountChange = (feeName, value) => {
    setStudentDataForm((prev) => {
      const updatedPadiFee = prev.padiFee.map((fee) => {
        if (fee.name === feeName) {
          const enteredAmount = parseFloat(value) || 0;
          // Check against finalAmount, not due amount
          if (enteredAmount > fee.finalAmount) {
            toast.error(`Entered amount for ${fee.name} exceeds the final amount.`);
            return fee;
          }
          return {
            ...fee,
        ...
        <form onSubmit={handleSubmit} className="p-5">
          <div className="mb-5">
            ...
            <table className="w-full border-collapse border text-left">
              <thead>
                <tr className="bg-gray-100">
                  <th className="p-2 border text-gray-900">Fee Type</th>
                  <th className="p-2 border text-gray-900">Final Amount</th>
                  <th className="p-2 border text-gray-900">No. of Terms</th>
                  <th className="p-2 border text-gray-900">Paid Amount</th>
                  <th className="p-2 border text-gray-900">Due</th>
                  <th className="p-2 border text-gray-900">Enter Amount</th>
                </tr>
              </thead>
              <tbody>
                {studentDataForm.padiFee.map((fee) => (
                  <tr key={fee.name}>
                    <td className="p-2 border text-gray-900">{fee.name}</td>
                    <td className="p-2 border text-gray-900">{fee.finalAmount}</td>
                    <td className="p-2 border text-gray-900">{fee.terms}</td>
                    <td className="p-2 border text-gray-900">{fee.paidAmount}</td>
                    <td className="p-2 border text-gray-900">{fee.due}</td>
                    <td className="p-2 border">
                      <input
                        type="number"
