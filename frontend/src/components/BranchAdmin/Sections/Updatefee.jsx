...
//               {fees.map((fee, index) => (
//                 <div key={index} className="flex mb-4 text-white">
...
//                     </select>
//                   </div>
//                   <div className="w-1/2 ml-2 text-white">
...
const UpdateFeeModal = ({ section, onClose, refreshSections, feeTypes }) => {
  ...
  // Add Another Fee Entry (only if unselected feeTypes remain)
  const handleAddFeeEntry = () => {
    if (fees.length < feeTypes.length) {
      setFees([...fees, { feeType: "", amount: "" }]);
    }
  };
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg modal-animation">
            ...
            <form onSubmit={handleSubmit} className="text-gray-700">
                ...
                <div key={index} className="flex mb-4 text-white">
                  <div className="w-1/2 mr-2">
                    ...
                    >
                      ...
                      {feeTypes
                        .filter((feeType) => {
                          const selectedFeeTypes = fees.map((f) => f.feeType);
                          return (
                            !selectedFeeTypes.includes(feeType.name) ||
              ...
              {/* Add Another Fee Button (only if available fee types remain) */}
              {fees.length < feeTypes.length && (
                <button
                  type="button"
                  onClick={handleAddFeeEntry}
                  className="mb-4 px-3 py-1 bg-gray-300 text-black rounded-md hover:bg-gray-400 transition"
